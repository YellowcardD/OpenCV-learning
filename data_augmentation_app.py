...
def parse_args():
    ...
    args.input_dir = args.input_dir.rstrip('/')  # 去掉末尾的/
...
def generate_image_list(args):
    ...
    # 计算平均处理的数目并向下取整
    num_avg_aug = int(math.floor(args.num/num_imgs))

    # 剩下的部分不足平均分配到每一个文件，所以做成一个随机幸运列表
    # 对于幸运的文件就多增加一个， 凑够指定的数目
    rem = args.num - num_imgs * num_avg_aug
    lucky_seq = [True] * rem + [False] * (num_imgs - rem)
    random.shuffle(lucky_seq)

    # 根据平均分配和幸运表策略， 生成每个文件的全路径和对应要增加的数目并放到一个list里
    img_list = [
        (os.sep.join([args.input_dir, filename]), num_avg_aug+1 if lucky else num_avg_aug) # os.sep 可以取代操作系统特定的路径分割符。
    ...
    for filepath, n in filelist:
        ...
        for i in range(n):
            ...
            if random.random() < args.p_crop:
                img_varied = random_rotate(img_varied, args.rotate_angle_vari, args.p_rotate_crop)
                varied_imgname += 'r'

            # 按照比例随机对图像进行HSV扰动
            if random.random() < args.p_hsv:
                img_varied = random_hsv_transform(img_varied, args.hue_vari, args.sat_vari, args.val_vari)
                varied_imgname += 'h'

            # 按照比例随机对图像进行Gamma扰动
            if random.random() < args.p_gamma:
                img_varied = random_gama_transform(img_varied, args.gamma_vari)
                varied_imgname += 'g'

            # 生成扰动后的文件名并保存在指定的路径
            output_filepath = os.sep.join([args.output_dir, '{}{}'.format(varied_imgname, ext)])
