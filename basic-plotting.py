...
# 基本绘图
canvas = np.zeros((400, 600, 3), dtype=np.uint8) + 255 #定义一块宽600高400的画布，初始化为白色
cv.line(canvas, (300, 0), (300, 399), (0, 0, 0), 2) #画一条纵向的正中央的黑色分界线
cv.line(canvas, (300, 149), (599, 149), (0, 0, 0), 2) #画一条右半部分画面以150为界的分界线
cv.circle(canvas, (200, 300), 75, (0, 0, 255), 5) #左半部分的右下角画个红色的圆
cv.rectangle(canvas, (20, 240), (100, 360), (255, 0, 0), thickness=3) #左半部分的左下角画个蓝色的矩形
triangles = np.array([                       #定义两个三角形，并执行内部绿色填充
...
#按像素为间隔从左至右在画面右半部分的上方画出HSV空间的色调连续变化
...
cv.imshow('Example of basic drawing functions', canvas)
cv.waitKey()
